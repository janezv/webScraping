...
<html>
    ...
    <div class="container">
        ...
        <h2>Prikaži zgodovino dnevne temperature</h2>
    ...
    <!-- glava tabele -->
        ...
        <tr><th><a href="/vreme/temper.php?sort=Dan">Dan</a></th><th><a href="/vreme/temper.php?sort=Kr">◊ Kredarica</a></th><th><a href="/vreme/temper.php?sort=Lj">◊ Ljubljana</a></th><th><a  href="/vreme/temper.php?sort=Kp">◊ Koper</a></th></tr>
        </div>
...
// izberi razvrstitev
    if(($_SESSION['count']%2==0) && ($_GET['sort']=="Dan")){
        ...
        $queryDB->queryTemper($histUSA, $nowUSA, $order);
    }
    elseif(($_SESSION['count']%2==1) && ($_GET['sort']=="Dan")){
        $order=" ORDER BY Dan ASC";
        $queryDB->queryTemper($histUSA, $nowUSA, $order);
    }
    elseif(($_SESSION['count']%2==0) && ($_GET['sort']=="Kr")){
        $order=" ORDER BY Kredarica DESC";
        $queryDB->queryTemper($histUSA, $nowUSA, $order);        
    }
    elseif(($_SESSION['count']%2==1) && ($_GET['sort']=="Kr")){
        $order=" ORDER BY Kredarica ASC";
        $queryDB->queryTemper($histUSA, $nowUSA, $order);        
    }
    elseif(($_SESSION['count']%2==0) && ($_GET['sort']=="Lj")){
        $order=" ORDER BY Ljubljana DESC";
        $queryDB->queryTemper($histUSA, $nowUSA, $order);        
    }
    elseif(($_SESSION['count']%2==1) && ($_GET['sort']=="Lj")){
        $order=" ORDER BY Ljubljana ASC";
        $queryDB->queryTemper($histUSA, $nowUSA, $order);        
    }
    elseif(($_SESSION['count']%2==0) && ($_GET['sort']=="Kp")){
        $order=" ORDER BY Koper DESC";
        $queryDB->queryTemper($histUSA, $nowUSA, $order);        
    }
    elseif(($_SESSION['count']%2==1) && ($_GET['sort']=="Kp")){
        $order=" ORDER BY Koper ASC";
        $queryDB->queryTemper($histUSA, $nowUSA, $order);        
    }
