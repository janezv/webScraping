...
<head>
    ...
    <script>
        function showPicture(ime){
                var img=ime.src;              
                window.open("../vreme/showOnePicture.php?img="+img,"_blank", "toolbar=no, scrollbars=yes, resizable=yes, top=500, left=500, width=850, height=650");
...
function showAllPictures(){
  ...
  //that I figure out with experiment, it just word, don't know why
  $st--;

  for ($st; 0 <= $st; $st--)
    {
        $image = $files[$st];
        $_SESSION['images'][$st] = $image;
        $fileEtension = array('jpg','png');
        $ext = pathinfo($image, PATHINFO_EXTENSION);
        if (in_array($ext, $fileEtension)) {   
            echo "<img id='$st' src='$image' height='200' width='250' onclick='showPicture(this)' style='cursor:pointer; margin-top:10'> &nbsp &nbsp";
        } 
    }
